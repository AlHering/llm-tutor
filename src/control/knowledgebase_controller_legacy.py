...
class KnowledgeBaseController(object):
    ...
    def __init__(self, peristant_directory: str, metadata: dict = None, base_embedding_function: EmbeddingFunction = None) -> None:
        ...
        self.default_client = chromadb.PersistentClient(
            ...
            settings=Settings(is_persistent=True, allow_reset=True))
        self.collections = {
    ...
    def _bypass_serialization_error(self, documents: List[Document]) -> Tuple[List[str], List[dict]]:
        """
        Temporary insert to bypass serializability error with Document objects (https://github.com/langchain-ai/langchain/issues/2446).
        :param documents: Documents.
        :return: Tuple of split up document content and metadata.
        """
    ...
    def embed_documents(self, name: str, documents: List[Document], ids: List[str] = None):
        ...
        """
        temporary_split = self._bypass_serialization_error(documents)
        self.collections.get(name, self.collections["base"]).add(documents=temporary_split[0], metadatas=temporary_split[1], ids=[
